...
export function BlogsTableRow({ row, type, selected, onSelectRow, onDeleteRow, onRestoreRow, onEditRow, onTrashRow }: BlogsTableRowProps) {
    ...
    <>
      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
        ...
        <TableCell component="th" scope="row">
          ...
          <Box gap={2} display="flex" alignItems="center">
            <Avatar alt={row.title} src={`${import.meta.env.VITE_APP_BASE_URL}/${row.image || ''}`} />
            <button
              type="button" // Added type attribute
              onClick={() => router.push(`/blog/${row._id}/edit`)}
              style={{ cursor: 'pointer', background: 'none', border: 'none', padding: 0, textAlign: 'left', color: 'inherit', font: 'inherit' }}
              aria-label={`View profile of ${row.title}`} // Accessible label
            >
              {row.title}
            </button>
          </Box>
